...
When the m3u8 extraction module returns multiple URLs with different resolutions, the UI SHALL display a modal dialog listing all available resolutions. The user MUST select one resolution before the download proceeds.

#### Scenario: Multiple resolutions available

- **WHEN** m3u8 extraction finds 3 URLs with labels "720p", "1080p", and "Unknown resolution"
- **THEN** the system SHALL display a modal dialog with radio buttons for each option
- **AND** the user SHALL select one option and confirm

#### Scenario: Single resolution available

- **WHEN** m3u8 extraction finds exactly 1 URL
- **THEN** the system SHALL NOT display the resolution selection dialog
- **AND** the download SHALL proceed automatically with the only available URL
